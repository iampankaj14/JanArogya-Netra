      ...
      <View className={`p-4 ${compact ? 'p-3' : 'p-4'}`}>
        <View className="flex-row items-center justify-between mb-2">
          <View className="flex-row items-center flex-1 mr-2">
            ...
            <Text className="font-bold text-slate-800 text-base ml-1.5 flex-1" numberOfLines={1}>
              {title}
            </Text>
          </View>
          <View className="px-2 py-0.5 rounded bg-slate-100">
            <Text style={{ color: priorityColor }} className="text-xxs font-bold uppercase text-[10px]">
              {priority}
            </Text>
          </View>
        </View>
        ...
        <Text className="text-slate-600 text-sm mb-3" numberOfLines={compact ? 2 : 3}>
          {description}
        </Text>
        ...
        <View className="flex-row items-center justify-between mt-1">
          ...
          {onPress && (
            <View className="flex-row items-center">
              <Text className="text-blue-900 font-bold text-xs mr-1">View Detail</Text>
              <Feather name={icons.chevronRight as any} size={12} color="#1E3A8A" />
            </View>
          )}
        </View>
      </View>
