...
async function applyMidnightEdits() {
  // Start with the COMMITTED version (9662d4f)
  let code = execSync('git show 9662d4f:app/(tabs)/situation-room.tsx', { cwd: '/Users/pankaj/Desktop/JanArogya Netra' }).toString();
  console.log(`Base committed file: ${code.length} chars, ${code.split('\n').length} lines`);
  ...
  for await (const line of rl) {
    try {
      ...
      // Apply ALL edits from 9898 to 10474 (midnight session)
        ...
        for (const call of parsed.tool_calls) {
          ...
          if (call.name === 'replace_file_content') {
            ...
            if (target !== undefined && code.includes(target)) {
              ...
              console.log(`  MISS at step ${parsed.step_index}: target not found`);
              failed++;
            }
            ...
            for (const chunk of call.args.ReplacementChunks) {
              ...
              if (target !== undefined && code.includes(target)) {
                ...
                console.log(`  MISS at step ${parsed.step_index} chunk: target not found`);
                failed++;
              }
            }
          }
  ...
  console.log(`Final: ${code.length} chars, ${code.split('\n').length} lines`);
  fs.writeFileSync('/Users/pankaj/Desktop/JanArogya Netra/situation_room_MIDNIGHT.tsx', code);
  console.log('Saved to situation_room_MIDNIGHT.tsx');
}
